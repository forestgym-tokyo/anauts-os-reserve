...
(()=>{
  ...
  q("#reservationForm")?.addEventListener("submit",async e=>{
    ...
    try{const list=await candidates();if(!list.length){showError("この時間に対応可能なトレーナーがいなくなりました。空き状況を更新してください。");return;}if(list.length===1){confirmedTrainerCode=String(list[0].staff_code||"");const summary=q("#selectedSlotText");if(summary&&!summary.textContent.includes("担当:"))summary.textContent+=` / 担当: ${trainerLabel(list[0])}`;announceFinalTrainer(list[0]);bypass=true;q("#reservationForm").requestSubmit();return;}chooseModal(list);}catch(err){showError(err.message||"担当トレーナーを確認できませんでした。");}finally{resolving=false;if(submit){submit.disabled=false;submit.textContent=old||"この内容で予約する";}}
  },true);
