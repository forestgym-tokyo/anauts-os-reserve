...
function sendMpgWithdrawalMemberMail_(member, data) {
  ...
  const body = [
    ...
    "",
    "【同意事項】",
    "同意済み：継続条件付きキャンペーンでのご入会／パーソナルトレーニング購入で継続条件が満たない場合、キャンペーン価格は適用されず通常価格との差額の精算が必要となります。",
    "同意済み：記載の退会期日が、入会時キャンペーンの定めで退会できない期間の場合、退会可能な最短期日をもって退会となります。",
    "同意済み：在籍中・退会後に虚偽や一方的な情報を拡散した場合、当ジムは必要に応じて法的措置を含む対応を行う可能性があることを承知します。",
    "",
