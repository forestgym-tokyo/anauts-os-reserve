(()=>{
  ...
  function ensureStyle(){
    ...
    s.textContent+=`
      .staff-reservation-row .withdrawal-link-actions{display:flex!important;flex-direction:column!important;align-items:stretch!important;justify-content:center!important;gap:8px!important;width:158px!important;min-width:158px!important;max-width:158px!important;flex-wrap:nowrap!important}
      .staff-reservation-row .withdrawal-link-actions>button{box-sizing:border-box!important;width:100%!important;min-width:0!important;height:44px!important;min-height:44px!important;padding:8px 12px!important;border-radius:10px!important;font-size:13px!important;font-weight:800!important;line-height:1.25!important;white-space:nowrap!important;margin:0!important}
      .staff-reservation-row .withdrawal-link-actions>.withdrawal-form-shortcut{border:1px solid #ff6673!important;background:#ff4f63!important;color:#fff!important}
      @media(max-width:700px){.staff-reservation-row .withdrawal-link-actions{width:100%!important;min-width:0!important;max-width:none!important}.staff-reservation-row .withdrawal-link-actions>button{width:100%!important;min-width:0!important}}
    `;
  ...
  function installStaffRoleWarning(){const form=q("#staffForm");if(!form||form.dataset.opsRoleCheck)return;form.dataset.opsRoleCheck="1";form.addEventListener("submit",e=>{if(OPS.staffSaveBypass){OPS.staffSaveBypass=false;return;}const code=String(q("#staffCode")?.value||"").trim().toUpperCase(),old=state.staff?.find(s=>String(s.staff_code)===code);if(!old)return;const changed=String(old.role||"")!==String(q("#staffRole")?.value||"")||bool(old.can_personal)!==q("#staffCanPersonal")?.checked||bool(old.can_tour)!==q("#staffCanTour")?.checked||bool(old.can_counsel)!==q("#staffCanCounsel")?.checked||bool(old.can_meal_planning)!==q("#staffCanMealPlanning")?.checked||bool(old.can_procedure)!==q("#staffCanProcedure")?.checked||bool(old.can_unsubscribe)!==q("#staffCanUnsubscribe")?.checked||bool(old.can_training_support)!==q("#staffCanTrainingSupport")?.checked||isActive(old)!==q("#staffActive")?.checked;if(!changed)return;e.preventDefault();e.stopImmediatePropagation();const ok=confirm("このスタッフ設定の変更後、今後の予約担当を再判定します。\n\nこのまま変更しますか？");if(!ok)return;OPS.staffSaveBypass=true;form.requestSubmit();},true);}


  function wrapApiPost(){if(OPS.apiWrapped||typeof apiPost!=="function")return;const original=apiPost;apiPost=async function(payload){let p=payload;if(p&&p.action==="createShiftChangeRequest"&&OPS.pendingConflictText){const warning=`【重要：既存予定あり】\n${OPS.pendingConflictText}`;p={...p,reason:[warning,p.reason||""].filter(Boolean).join("\n\n"),has_existing_reservation:true,existing_reservation_summary:OPS.pendingConflictText};OPS.pendingConflictText="";}const result=await original(p);if(p&&["saveStaff","setStaffActive"].includes(p.action)){OPS.staff=[];}if(p&&["saveService","setServiceActive"].includes(p.action)){OPS.services=[];}if(p&&["saveStaffShift","deleteStaffShift","createShiftChangeRequest"].includes(p.action)){OPS.shifts=[];}if(p&&["saveStaff","setStaffActive","saveService","setServiceActive","saveStaffShift","deleteStaffShift","createShiftChangeRequest","updateReservation","reassignReservationStaff"].includes(p.action)&&q("#operationsTopView.is-active")){setTimeout(()=>loadTop(true),700);}return result;};OPS.apiWrapped=true;}
  ...
  let uiEnhanceTimer=0;
  function scheduleUiEnhancement(){if(uiEnhanceTimer)return;uiEnhanceTimer=setTimeout(()=>{uiEnhanceTimer=0;enhanceCalendarDetails();fixWithdrawalLayout();installShiftWarnings();installStaffRoleWarning();},80);}
  function boot(){if(typeof state==="undefined"||typeof apiGet!=="function"||typeof apiPost!=="function"||typeof localYmd!=="function"){setTimeout(boot,100);return;}buildTop();wrapApiPost();wrapPermissionUi();installShiftWarnings();installStaffRoleWarning();if(state.authUser)q("#operationsTopNav")?.classList.remove("is-hidden");const mo=new MutationObserver(scheduleUiEnhancement);mo.observe(document.documentElement,{childList:true,subtree:true});q("#logoutButton")?.addEventListener("click",()=>q("#operationsTopNav")?.classList.add("is-hidden"));}
