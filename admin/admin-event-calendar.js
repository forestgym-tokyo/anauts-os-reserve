(()=>{
  ...
  function rawServiceName(r){return String(r.service_name||r.service_code||"予約").trim();}
  function calendarServiceName(r){
    const name=rawServiceName(r),upper=name.toUpperCase();
    ...
    if(name.includes("ミールフィット")||upper.includes("MEALFIT")||upper.includes("MEAL_FIT"))return"食事相談";
    if(name.includes("トレーニングサポート")||upper.includes("TRAINING_SUPPORT"))return"トレサポ";
    if(name.includes("パーソナル"))return"パーソナル";
    if(name.includes("無料体験")||upper.includes("TRIAL"))return"無料体験";
    return name;
  }
  function serviceColor(r){
    const name=rawServiceName(r),upper=name.toUpperCase();
    if(name.includes("店内見学")||upper.includes("TOUR"))return"#3B82F6";
    if(name.includes("退会")||upper.includes("UNSUBSCRIBE")||upper.includes("WITHDRAW"))return"#EF4444";
    if(name.includes("諸手続")||upper.includes("PROCEDURE"))return"#64748B";
    if(name.includes("ダイエット")&&name.includes("カウンセリング"))return"#F59E0B";
    if(name.includes("ミールフィット")||upper.includes("MEALFIT")||upper.includes("MEAL_FIT"))return"#22C55E";
    if(name.includes("トレーニングサポート")||upper.includes("TRAINING_SUPPORT"))return"#8B5CF6";
    if(name.includes("パーソナル"))return"#166534";
    if(name.includes("無料体験")||upper.includes("TRIAL"))return"#06B6D4";
    return"#24483b";
  }
  ...
  function render(result){
    ...
    for(let i=0;i<total;i++){
      const day=i-offset+1;if(day<1||day>meta.last){cells+='<div class="ecal-day out"></div>';continue;}
      const date=`${eventState.month}-${String(day).padStart(2,"0")}`,items=byDate.get(date)||[];
      const visible=items.slice(0,3).map(r=>`<span class="ecal-event" style="background:${serviceColor(r)}"><b>${html(String(r.start_time||"").slice(0,5))} ${html(calendarServiceName(r))}</b><span>${html(r.customer_name||"氏名未登録")}</span></span>`).join("");
      const more=items.length>3?`<span class="ecal-more">＋${items.length-3}件</span>`:"";
      cells+=`<div class="ecal-day ${date===today?"today":""} ${date===eventState.selectedDate?"is-selected":""}" data-ec-date="${date}"><div class="ecal-num">${day}</div>${visible}${more}</div>`;
    }
