...
function calc(){
  ...
  const items=[];
  items.push({target:"—",label:"初期費用",paid:initialPaid,normal:8800,settlement:Math.max(0,8800-initialPaid),note:"入会金・事務手数料",kind:"INITIAL"});
  const benefitValue=benefitAmount(benefit,rate,joinDate,plan);
  if(benefitValue>0)items.push({target:"—",label:"選べる特典",paid:0,normal:benefitValue,settlement:benefitValue,note:benefit==="PERSONAL"?"無料パーソナル":"2か月目無料",kind:"BENEFIT"});
  months.forEach((x,i)=>{
    const target=`${x.y}年${x.m+1}月`;
    if(i===0){
      items.push({target,label:"初月会費",paid:firstPaid,normal:firstNormal,settlement:Math.max(0,firstNormal-firstPaid),note:"日割り差額",kind:"MONTH",isFinalMonth:months.length===1,status:"通常"});
      return;
    }
    const isFinal=i===months.length-1;
    let paid=rate.campaign;
    if(isFinal&&cut.beforeFinalCharge)paid=0;
    items.push({target,label:"月会費",paid,normal:rate.normal,settlement:Math.max(0,rate.normal-paid),note:`${i}/12`,kind:"MONTH",isFinalMonth:isFinal,status:"通常"});
  });
...
function render(){
  ...
  current.items.forEach((x,i)=>{
    ...
    const status=x.kind==="MONTH"?`<select data-status="${i}"><option value="通常">通常</option><option value="休会">休会</option></select>`:"—";
    tr.innerHTML=`<td>${x.target}</td><td>${status}</td><td>${x.label}</td><td class="num">${yen(x.paid)}</td><td class="num">${yen(x.normal)}</td><td class="num"><strong>${yen(x.settlement)}</strong></td>`;
    body.append(tr);
  });
  body.querySelectorAll("[data-status]").forEach(sel=>sel.addEventListener("change",e=>{
    ...
    if(item.status==="休会"){item.paid=550;item.normal=550;item.settlement=0;item.note="休会期間（差額なし・縛り延長）";}
    else{
      const rate=rateFor(current.plan,current.joinDate);
      if(i===current.items.findIndex(z=>z.kind==="MONTH")){
        item.paid=Number($("#settlementFirstPaid").value||0); item.normal=Number($("#settlementFirstNormal").value||0);
      }else{
        item.normal=rate.normal;
        item.paid=item.isFinalMonth&&cutoffInfo().beforeFinalCharge?0:rate.campaign;
      }
      item.settlement=Math.max(0,item.normal-item.paid);
    }
    render();
  }));
