...
for (const html of [indexHtml, adminHtml]) {
  ...
  assert.match(html, /firebase-config\.js\?v=20260918-shift-publication-v1/);
}
...
const context = {
  ...
  state: {
    ...
    staffSchedule: {
      ...
      reservations: [
        { reservation_id: "tour", service_code: "TOUR", store_code: "YACHIYO", staff_code: "ABSENT", start_time: "10:00", end_time: "11:00", status: "RESERVED" },
        { reservation_id: "cross-store-tour", service_code: "TOUR", store_code: "YACHIYO", staff_code: "CROSS", start_time: "10:00", end_time: "11:00", status: "RESERVED" },
        { reservation_id: "procedure", service_code: "PROCEDURE", store_code: "YACHIYO", staff_code: "", start_time: "11:00", end_time: "11:30", status: "RESERVED" },
        { reservation_id: "unsubscribe", service_code: "UNSUBSCRIBE", store_code: "YACHIYO", staff_code: "ABSENT", start_time: "12:00", end_time: "12:10", status: "RESERVED" },
...
(async () => {
  ...
  assert.deepEqual(
    ...
    ["tour", "cross-store-tour", "procedure", "unsubscribe", "support"]
  );
  assert.deepEqual(
    calls.map(payload => payload.service_code),
    ["TOUR", "TOUR", "PROCEDURE", "UNSUBSCRIBE", "TRAINING_SUPPORT45"]
  );
  ...
  assert.equal(calls.length, 5, "The same unchanged schedule must not be posted repeatedly");
