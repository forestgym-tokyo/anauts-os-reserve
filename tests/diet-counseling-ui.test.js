...
const createReservationCaseIndex = main.indexOf('case "createReservation":');
const dietCounselingRouteIndex = main.indexOf(
...
assert.match(main, /getDietCounselingClientSheet_\(params\)/);
assert.match(main, /submitDietCounselingResponse_\(body\)/);
assert.match(main, /issueDietCounselingClientViewForAdmin_\(body\)/);
...
assert.match(form, /client_token/);
assert.match(form, /getDietCounselingClientSheet/);
assert.match(form, /issueDietCounselingClientView/);
...
assert.match(form, /!TOKEN && !ADMIN_VIEW_TOKEN && !CLIENT_VIEW_TOKEN && !IS_SHEET_PREVIEW && !IS_PRINT_PREVIEW/);
...
assert.match(formHtml, /id="copyClientLinkButton"/);
assert.match(formHtml, /id="printPage"/);
assert.match(formHtml, />管理者用</);
assert.match(formHtml, /お客様用URLをコピー/);
...
assert.match(formStyles, /body\.is-client-sheet-view/);
...
assert.match(formHtml, /app\.js\?v=20260918-client-view-v1/);
